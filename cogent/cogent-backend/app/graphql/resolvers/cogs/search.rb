class Resolvers::Cogs::Search < Resolvers::BaseSearchResolver
  ...
  class UserEnum < Types::BaseEnum
    graphql_name 'UserEnum'

    value 'ME'
    value 'ALL_WITHOUT_ME'
    value 'ALL'
  end

  scope { Cog.includes(:cards, :blocks).where(is_public: true).where(id: Cog.joins(blocks: :cards).select(:id)) }

  option :user, type: UserEnum, default: 'ALL'
